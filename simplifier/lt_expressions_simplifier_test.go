...
func (s *LtExpressionsSimplifierSuite) Test_simplifyLTExpression(c *C) {
	sx := createLtExpressionsSimplifier().Simplify(tree.Comparison{Op: tree.LT, Left: tree.NumericLiteral{42}, Right: tree.NumericLiteral{15}})

	c.Assert(tree.ExpressionString(sx), Equals, "(gte 15 42)")
}

func (s *LtExpressionsSimplifierSuite) Test_simplifyLTEExpression(c *C) {
	sx := createLtExpressionsSimplifier().Simplify(tree.Comparison{Op: tree.LTE, Left: tree.NumericLiteral{43}, Right: tree.NumericLiteral{16}})
